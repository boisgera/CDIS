 ...
 * Dans beaucoup de cas de figure, on fait intervenir l'aléatoire dans la modélisation du fait d'une connaissance incomplète des phénomènes. C'est le cas par exemple en sciences du climat 
...
### Exemples {.example}
 ...
 5. Evolution du prix d'un actif financier sur un intervalle de temps $[t_1,t_2] \subset \R^+$ : 
 $A = \{ f \in C([t_1,t_2], \R^+) : \|f-g\|_{\infty} \leq a \}$, où
 $g \in C([t_1,t_2],\R^+)$ et $a \in \R^+$.
...
On vérifie aisément que $\P$ vérifie les 3 propriétés suivantes.
 ...
 3. Si $A_n$ désigne une suite (dénombrable) d'événements **disjoints** de $\A$, on a en utilisant le théorème de convergence monotone à la suite croissante de fonctions $1_{\left\{\bigcup_{n=0}^m A_n\right\}} = \sum_{n=0}^m 1_{A_n}$ ($m \in \N^\ast$) :
    \begin{align*}
...
Une *probabilité* sur l'espace $(\Omega, \A)$ est une application $\P : \A \rightarrow [0,1]$, telle que :
 ...
 1. $\P(\Omega) = 1$,
 2. pour toute suite (dénombrable) $(A_n)_{n\in\N}$ d'éléments de $\A$ **deux à deux disjoints**, on a 
 \begin{equation}
