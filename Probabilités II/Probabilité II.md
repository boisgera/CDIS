...
### Remarque {.remark}
 Comme $F$ est croissante, elle admet une limite à gauche en chaque point notée $F(x-)$. En remarquant que $]-\infty,y[ = \lim\limits_{n \to +\infty}$ si $y_n$ tend vers $y$ par valeurs décroissantes, on obtient pour 
 $x< y$: 
...
### Théorème {.theorem}
Si $F$ est une fonction réelle sur $\R$ qui vérifie les trois conditions de la [proposition](#propfdr), c'est la fonction de répartition d'une (unique) probabilité $\P$ sur $\R$ munie de la tribu borélienne $\B(\R)$. On ne peut pas, en général, définir $\P$ sur la tribu $\mathcal{P}(\R)$ de toutes les parties de $\R$.
