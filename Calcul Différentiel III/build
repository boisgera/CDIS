...
# ------------------------------------------------------------------------------
"""
  - rename targets/outputs "print" vs "e-book" (do not distinguish print/letter ?)

  - for the print target, work the local links to display a page and rework
    the link to answers ? (a rework for the e-book version may be in order too)
    ...
    for elt, path in pandoc.iter(doc, path=True):
        if isinstance(elt, Header):
            ...
            if shared:
                ...
                if "two" in classes:
                    inlines += [Space(), Str("("), Math(InlineMath(), r"\mathord{\bullet}"*2), Str(")")] 
                if "three" in classes:
